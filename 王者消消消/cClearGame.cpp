...
int cClearGame::CreateMonster(MonsterType type)
{
	...
	{
		monster.m_ATK = 10;
		monster.m_life = 10 * m_currentWaveTimes;
		monster.m_maxFps = 8;
	}
	else
	{
		monster.m_ATK = 15;
		monster.m_life = 10 * m_currentWaveTimes + 100;
		monster.m_maxFps = 23;
	}
...
int cClearGame::DeleteBooms()
{
	for (list<cBullet>::iterator it = m_bulletList.begin(); it != m_bulletList.end();)
	{
		if (it->m_isDie == TRUE)
		{
			if (it->m_type == Bullet_Dragon)
			{
				m_currentBlood >= 0 ? m_currentBlood -= it->m_ATK/2 : m_currentBlood;
				if (m_currentBlood <= 0)
				{
		...
		if (it->m_x < 1133)
		{
			...
			if (distance < 160)
			{
				if (defense.m_ATKDelay == 0)
				{
					...
					{
						--m_CurrentBoomNums;
						ShootBullet(defense.m_x + 100, defense.m_y + 70, Bullet_Defense, defense.m_ATK, defense.m_speed+3, NULL, it->m_id);
					}
				}
				else
				{
